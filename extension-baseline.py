...
# -*- coding: utf-8 -*-
"""
extension-baseline.py
---------------------
BiDAF model extension with pre-trained GloVe embeddings.

This extension replaces the random embedding layer in the strong baseline
with 100-dimensional GloVe word vectors to improve generalization.
...
def main():
    ...
    ap.add_argument("--glove", default="data/glove.6B.100d.txt", help="Path to GloVe embedding file.")
    args = ap.parse_args()
    ...
    if os.path.exists(args.glove):
        ...
        print("Warning: GloVe file not found. Using random embeddings.")
        pretrained_emb = None
    ...
    model = BiDAF(vocab_size=len(vocab), pretrained_emb=pretrained_emb)

    print(" Training BiDAF (GloVe version) on train_v2.json...")
